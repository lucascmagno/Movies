...
public class CadastroDao {
	public boolean inserirUsuario(LoginCadastro c) {
		...
		try {
			con = new Conexao();
			con.executeUpdate("INSERT INTO login(nome, sobrenome, usuario, email, senha) VALUES('"+c.getNome()+"'"
					+ ",'"+c.getSobrenome()+"',"
					+ "'"+c.getUsuario()+"',"
					+ "'"+c.getEmail()+"',"
					+ "'"+c.getSenha()+"') ;");
	...
	public LoginCadastro login (String usuario, String senha) {
		...
		try {
			con = new Conexao();
			ResultSet rs = con.executeQuery("SELECT * FROM login WHERE usuario = '"+usuario+"' AND senha='"+senha+"';");
